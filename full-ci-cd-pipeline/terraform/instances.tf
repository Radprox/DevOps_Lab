...
ansible-playbook --extra-vars 'passed_in_hosts=tag_Name_${self.tags.Name}' playbook-master.yml
EOF
  }
}
...
ansible-playbook --extra-vars 'passed_in_hosts=tag_Name_${self.tags.Name} master_ip=${aws_instance.jenkins-master.private_ip}' playbook-worker.yml
EOF
  }
